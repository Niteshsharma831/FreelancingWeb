// controllers/freelancerController.ts
...
// Middleware to protect freelancer routes
...
// Get a single freelancer by email (or use req.user.id if authenticated)
...
export const getFreelancerProfile = async (req: Request, res: Response) => {
  try {
    const freelancerId = (req as any).user?.id; // ✅ FIXED here
