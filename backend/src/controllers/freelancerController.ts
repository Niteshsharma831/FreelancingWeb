...
interface AuthUser {
  id: string;
  role: string;
  email: string; // Email is REQUIRED, not optional
}
...
// Register Freelancer
export const registerFreelancer = async (req: Request, res: Response): Promise<void> => {
  try {
    const { name, gender, email, otp } = req.body;
...
// Login
export const loginFreelancer = async (req: Request, res: Response): Promise<void> => {
  try {
    const { email, otp } = req.body;
...
// Protect middleware
export const protectFreelancer = (req: AuthRequest, res: Response, next: NextFunction): void => {
  try {
    const token = req.cookies?.token || req.headers.authorization?.split(" ")[1];
    const JWT_SECRET = process.env.JWT_SECRET || "your_jwt_secret";
...
// Update Profile
export const updateFreelancer = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { profile } = req.body;
...
export const getFreelancer = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { email } = req.query;
    const userId = req.user?.id;
...
export const getFreelancerProfile = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
