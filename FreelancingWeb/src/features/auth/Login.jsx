...
const LoginPage = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      await api.post("/api/users/create", { name, gender, email, otp });
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await api.post("/api/users/login", { email, otp });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-pink-100 to-orange-100 text-gray-800 flex items-center justify-center px-4 py-12">
      <ToastContainer />
      <div className="max-w-7xl w-full flex flex-col lg:flex-row items-center justify-between gap-12">
        <div className="text-center lg:text-left max-w-xl flex-shrink-0">
          <h1 className="text-4xl md:text-5xl font-extrabold mb-4 text-gray-900">
            Big Brands Hiring Drive <span className="text-pink-600">2025</span>
          </h1>
          <p className="text-lg md:text-xl mb-6">
            Choose your next job with India's most reputed companies.
          </p>
